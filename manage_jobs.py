...
def create_job(apiToken, serverURL, accountID, datastreamID, jobType, entityCol, timeIdentifier, timeFormat, timeZone):
  ...
  r = requests.post(serverURL + "/api/1.1/accounts/%s/jobs" %(accountID), data=dataValue, headers=authToken)
  rdict = r.json()
...
def get_jobs(apiToken, serverURL, accountID, datastreamID, jobType, jobStatus):
  r = {}
  authToken = {"Authorization" : "Bearer "+apiToken}
  r = requests.get(serverURL + "/api/1.1/accounts/"+str(accountID)+"/jobs?limit=100&type="+jobType+"&status="+jobStatus+"&datastream="+str(datastreamID), headers=authToken)
  rdict = r.json()
  ids=[]
  for k in rdict:
    ids.append(k['id'])
